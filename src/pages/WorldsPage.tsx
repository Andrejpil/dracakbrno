...
import { Trash2, Plus, Crown, Check, UserPlus } from 'lucide-react';
import { toast } from 'sonner';

interface Member { world_id: string; user_id: string; }
...
export default function WorldsPage() {
  ...
  async function invite(worldId: string) {
    ...
    const { error } = await supabase.from('world_members' as any).insert({
      world_id: worldId, user_id: p.id, member_role: 'viewer',
    });
    if (error) { toast.error(error.message); return; }
    setInviteEmail('');
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        Každý svět má vlastní obsah. Zde určíte, <strong>kdo do světa vstoupí</strong>. Co v něm smí dělat
        (prohlížet / editovat / spravovat) se řídí globální rolí z karty <strong>Uživatelé</strong>.
      </p>
                ...
                <div className="mt-4 pt-4 border-t space-y-3">
                  <div className="flex items-end gap-2 flex-wrap">
                    <div className="flex-1 min-w-[220px]">
                      <Label className="text-xs">Přidat uživatele podle e-mailu</Label>
